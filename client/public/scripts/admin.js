...
$("#generate_report_form").submit((e) => {
	...
	switch ($("#report_type").val()) {
		case "General Region Report":
			url = "/generate/type1/report";
			break;
		case "General Region Report Sorted by ICC":
	...
	var icc = $("#report_icc").children(":selected").attr("id");
	$.post(url, { regions: regions, forwardPeriod: $("#report_forward").val(), icc_type: $("#report_icc_type").val(), icc: icc }, (data) => {
		if (data.redirect) window.location.href = data.redirect;
		if (data.file) {
			window.open(`/tmp/${data.file}`, "_blank");
			window.focus();
		}
...
function updateInfo() {
	switch ($("#report_type").val()) {
		case "General Region Report":
			$("#info").text("This report will find all the expiring/expired/allocated ICCs and list them per employee");
			$("#icc_group").hide();
			$("#forward_group").show();
			break;
		case "General Region Report Sorted by ICC":
			$("#info").text("This report will find all the expiring/expired/allocated ICCs and list them per ICC");
			$("#icc_group").hide();
			$("#forward_group").show();
			break;
		case "ICC Report":
			$("#info").text("This report will find every employee in a region with or without an ICC and sort them into those groups");
			$("#icc_group").show();
			$("#forward_group").hide();
			break;
		case "Qualification Report":
			$("#info").text("This report will find all qualifications and list them per employee");
			$("#icc_group").hide();
			$("#forward_group").hide();
			break;
		default:
...
function updateICCType() {
	switch ($("#report_icc_type").val()) {
		case "Site Induction":
			$("#report_icc").empty("");
			$("#report_icc").append("<option selected>Select ICC</option>");
			$.post("/get/site/templates", {}, (data) => {
		...
		case "Product Certification":
			$("#report_icc").empty("");
			$("#report_icc").append("<option selected>Select ICC</option>");
			$.post("/get/product/templates", {}, (data) => {
				HandleTemplates(data);
			});
			break;
		case "Health and Safety Qualification":
			$("#report_icc").empty("");
			$("#report_icc").append("<option selected>Select ICC</option>");
			$.post("/get/health/templates", {}, (data) => {
				HandleTemplates(data);
			});
			break;
		case "General Certification":
			$("#report_icc").empty("");
			$("#report_icc").append("<option selected>Select ICC</option>");
			$.post("/get/certification/templates", {}, (data) => {
				HandleTemplates(data);
			});
			break;
		case "Qualification":
			$("#report_icc").empty("");
			$("#report_icc").append("<option selected>Select ICC</option>");
			$.post("/get/qualification/templates", {}, (data) => {
				HandleTemplates(data);
			});
			break;
		default:
...
function HandleTemplates(data) {
	...
	for (var i = 0; i < data.templates.length; i++) {
		$("#report_icc").append(`<option id=${data.templates[i].id}>${data.templates[i].name}</option>`);
	}
}
function clearReportForm() {
	...
	toggled = false;
	$("#report_icc").val("Select ICC");
	$("#report_icc_type").val("Select ICC Type");
	updateInfo();
}
