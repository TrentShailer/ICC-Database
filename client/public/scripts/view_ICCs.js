...
$("#edit_form").submit((e) => {
	...
	$.post(`/edit/employee/${type}/icc`, { id: selectedID, icc: newICCID, training_date: isoDate }, (data) => {
		if (data.redirect) window.location.href = data.redirect;
			...
			$.post(`/get/${type}/templates`, (data) => {
				...
				templates.forEach((template) => {
					if (template.id == template_id) {
						var html = `<option data-duration='${template.duration}' selected id=${template.id}>${template.name}</option>`;
						$("#duration").val(template.duration + " Months");
					} else {
...
function calculateExpiration() {
	var text_date = $("#edit_training_date").val();
	var duration = $("#edit_icc").children(":selected").data("duration");
