...
app.post("/generate/type2/report", urlencodedParser, async (req, res) => {
	if (req.session.user && req.session.user.admin_access) {
		...
		var outputName = `Region ICC Report ${format(now, "yyyy-MM-dd hh-mm a")}.pdf`;
		...
		pdf
			.create(buffer, {
				...
				header: {
					...
					<p class="header" id="title">General Region Report</p>
				</div>
				<div style="margin-top: -15px">
					...
					<div style="float: right; text-align: right;">
						<p class="info">Report Type: ICC Expiring/Expired/Allocated</p>
					</div>
...
async function GenerateRegion(region_id, region_name, forwardPeriod) {
	...
	for (var i = 0; i < userQuery.rows.length; i++) {
		...
		rows = rows.concat(await GetICCs(user_id, forwardPeriod, name));
	}
	// Sort all ICCs by name
	rows.sort((a, b) => {
		var nameA = a.icc.toUpperCase();
		var nameB = b.icc.toUpperCase();
		if (nameA < nameB) {
			return -1;
		}
		if (nameA > nameB) {
...
async function GetICCs(user_id, forwardPeriod, name) {
	var rows = [];
	...
	if (!(siteQuery < 0)) {
		for (var i = 0; i < siteQuery.rows.length; i++) {
			...
			if (validateRow(dataRow, forwardPeriod) && (dataRow.duration != "" || dataRow.duration != null)) {
				...
				rows.push({ icc: dataRow.name, expiration_date: expiration_date, name: name });
			}
		}
	}
