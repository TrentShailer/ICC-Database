...
app.post("/get/employee/site/iccs", urlencodedParser, async (req, res) => {
	if (req.session.user) {
		...
		for (var i = 0; i < siteQuery.rows.length; i++) {
			...
			var training_date = row.training_date == null ? "" : format(row.training_date, "dd/MM/yyyy");
			var expiration_date = row.expiration_date == null ? "" : format(row.expiration_date, "dd/MM/yyyy");
...
async function getICCData(req, res, sql) {
	...
	res.send({
		training_date: query.rows[0].training_date == null ? "" : format(query.rows[0].training_date, "dd/MM/yyyy"),
		template_id: query.rows[0].template_id,
	});
}
...
async function handleICCEdit(req, res, sql, templateSQL) {
	var id = req.body.id;
	var training_date = req.body.training_date == "" ? null : req.body.training_date;
	var icc = req.body.icc;
